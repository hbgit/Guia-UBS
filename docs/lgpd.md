...
|---|---|---|
| LGPD-RT01 | APIs | Nenhum dado pessoal em URL/query string; autenticação em todo endpoint do CMS; respostas de erro sem dados de titulares (mensagem genérica + ID opaco no log servidor). **Duas exceções declaradas: `GET /health` e `POST /api/telemetry`** (esta última porque autenticar exigiria identidade de dispositivo, que a INV-2 proíbe; há teste percorrendo `/api` e exigindo que nenhuma outra dispense sessão). `GET /health`, que responde uma constante e não toca no banco de identidade — é o healthcheck do compose. `onError` devolve mensagem genérica; validação de corpo não ecoa o recebido, que carregaria a senha de volta na resposta |
| LGPD-RT02 | Banco de dados | Colunas de dado pessoal do CMS marcadas no schema; acesso de produção restrito e logado; sem dado pessoal no `content.db` distribuído (por construção). **SQLite/libSQL não tem `COMMENT` de coluna** — o equivalente executável é `PII_COLUMNS` em `cms/src/db/schema/index.ts`, e `cms/test/invariants.test.ts` reprova se qualquer coluna listada deixar de existir. Superfície atual: `admin_user.email`, `.name`, `.image` (exigida pelo core do Better Auth e **nunca escrita por rota nossa**), `account.password` (Argon2id), `two_factor.secret` e `.backup_codes` (cifrados em repouso), `session.ip_address` e `.user_agent` (PII de sessão viva, apagada no logout), `login_attempt.subject_key` e `audit_entry.ip_hash` (pseudonimizados com sal), `consent_record.subject_ref`. `audit_entry.before_json`/`after_json` carregam PII quando a entidade auditada é `admin_user` |
| LGPD-RT03 | Logs | Proibido dado pessoal e dado de saúde em logs; IDs opacos (UUID); ring buffer local do app sem PII (espelha INV-5 da spec); retenção de logs do CMS definida na tabela RF07. **`audit_entry` é append-only por gatilho** (`cms/src/db/triggers.sql`): `UPDATE` e `DELETE` recusados com `RAISE(ABORT)` citando este requisito, verificado em `cms/test/append-only.test.ts`. Só o IP **hasheado** é guardado |
...
| LGPD-RT05 | Registro de aceite | Tabela append-only `consent_record` (`subject_ref, doc_type, doc_version, doc_hash, method, accepted_at`) — `UPDATE`/`DELETE` recusados por **gatilho no banco**, não por checagem de rota: a proteção precisa valer também contra bug de ORM, script de manutenção e sessão comprometida com acesso ao shell. O `doc_hash` é copiado no momento do aceite, para o vínculo não depender de JOIN |
| LGPD-RT06 | Criptografia | TLS 1.2+ obrigatório; Ed25519 para packs; **Argon2id** para senhas do CMS (`@node-rs/argon2`, parâmetros OWASP m=19456/t=2/p=1, substituindo o scrypt padrão do Better Auth — `cms/test/password.test.ts` confere o prefixo `$argon2id$` e os parâmetros no próprio hash, para que downgrade de custo seja visível); segredo TOTP e códigos de recuperação cifrados em repouso pelo plugin sob `BETTER_AUTH_SECRET`; segredos por ambiente, e o processo **não sobe** sem eles |
| LGPD-RT07 | Sessões/autenticação | Better Auth com 2FA TOTP; expiração de sessão ≤ 24 h (`SESSION_MAX_AGE_SECONDS`, afirmado em teste); invalidação no logout; **bloqueio progressivo por CONTA** em `login_attempt` — 2 tentativas de folga, depois 30 s dobrando até o teto de 15 min, zerado no sucesso. Por conta e não por IP porque o rate limit embutido é janela fixa por endpoint e quem distribui tentativas entre IPs passa por baixo. Conta travada recusa até a senha **certa**, e a resposta não revela quanto falta nem quantas restam — seria um oráculo sobre conta alheia. O teto existe para o ataque não virar negação de serviço contra o próprio operador |
...
| LGPD-RT10 | Exclusão segura | Expurgo com evidência (log do job); dados em SSD gerenciado: exclusão lógica + criptografia em repouso como compensação; no device, "apagar meus dados" remove os arquivos do sandbox do app. **Colisão declarada com RT03/RT05:** o gatilho `BEFORE DELETE` é incondicional, então o expurgo por prazo (RF07) é procedimento **nomeado** que derruba e recria os gatilhos dentro de uma transação e registra a própria execução em `audit_entry`. Assim o caminho de aplicação continua incapaz de apagar — que é a ameaça real — sem tornar impossível a obrigação legal |
...
| LGPD-RF11 | Acesso | RBAC + 2FA no CMS | 3 papéis, segregação edição×aprovação, sessões expiráveis | P0 | Segurança | 46; 47 | **Implementado (item 17).** Matriz papel×permissão em `cms/src/auth/permissions.ts` como DADO, percorrida pelo produto cartesiano inteiro em `cms/test/rbac.test.ts`: `editor` não tem `approval:decide`, `clinical_reviewer` não tem `content:write`, `admin` não tem nenhum dos dois. 2FA **obrigatória** por middleware (o Better Auth a trata como opt-in), com uma exceção nomeada para o próprio cadastro do TOTP. Sem autocadastro. O plugin `admin` da biblioteca ficou fora por trazer impersonation, que deixaria um admin aprovar como se fosse o revisor. **A regra por LINHA (`approver_id ≠ created_by`) virou GATILHO no item 19** — mora no banco, não na rota, porque a ameaça nomeada no PRD é o insider e uma checagem só na rota protege apenas contra quem passa por ela. O quórum (≥ 1 `clinical_reviewer`) é agregado e vive em `services/approval-workflow.ts`. O cenário realmente alcançável é a **promoção**: um editor cria a release e é promovido a revisor — a matriz de papéis não vê, só a regra por linha barra |
...
| LGPD-RF13 | Dados sensíveis | Saúde só no device | Tokens de sintomas em memória; proibida persistência/transmissão | P0 | Dado sensível | 5º II; 11; 12 | Revisão de código + teste de rede + schema de telemetria em CI + **enumeração das colunas do `user.db` em CI** (`app/test/prefs/lgpd_surface_test.dart`): coluna nova reprova o build. Superfície atual, revisada: `locale_code`, `telemetry_enabled`, `allow_metered_download`, `setup_completed`, `theme_mode`, `font_scale` — todas escolhas de operação da interface; nenhuma identifica pessoa nem descreve saúde |
| LGPD-RF14 | Analytics | Anti-reidentificação | Sem IDs persistentes; k-anonimato ≥ 20; granularidade diária | P0 | Anonimização | 12; 6º III | **Endpoint implementado (item 19)**: `POST /api/telemetry` reusa `isAcceptableBatch` do contrato — allowlist e k≥20 num ponto de decisão só; `CHECK` no DDL vale para qualquer origem; `UNIQUE(coorte, dia)` recusa lote repetido em vez de somar em silêncio; nenhuma coluna de `telemetry_batch` identifica aparelho, e há teste que percorre as colunas. **Lacuna declarada: não há produtor.** O app não envia (3ª chamada de rede exige ADR) e, mesmo que enviasse, `TelemetryRecorder` acumula por APARELHO enquanto o contrato exige k≥20 no lote — falta um agregador, que nenhum documento nomeia |
