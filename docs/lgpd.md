...
|---|---|---|
| LGPD-RT01 | APIs | Nenhum dado pessoal em URL/query string; autenticação em todo endpoint do CMS; respostas de erro sem dados de titulares (mensagem genérica + ID opaco no log servidor). **Única exceção declarada: `GET /health`**, que responde uma constante e não toca no banco de identidade — é o healthcheck do compose. `onError` devolve mensagem genérica; validação de corpo não ecoa o recebido, que carregaria a senha de volta na resposta |
